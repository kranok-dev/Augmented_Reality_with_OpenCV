...
start = time.time()
frame = cv2.imread("images/image_4.jpg")        #Change image to read
frame = cv2.resize(frame,None,fx=resizeFactor,fy=resizeFactor, interpolation = cv2.INTER_AREA)
...
cv2.putText(originalFrame,"Original",(int(width*0.05),int(height*0.95)),cv2.FONT_HERSHEY_SIMPLEX,1.1,(150,0,0),3, cv2.LINE_8)
cv2.putText(frame,"Augmented Reality",(int(width*0.05),int(height*0.95)),cv2.FONT_HERSHEY_SIMPLEX,1.1,(0,150,0),3, cv2.LINE_8)
...
cv2.imshow("Results", combinedImage)

print("Time Elapsed: %7.4fs  FPS: %7.4f\n" %(time.time()-globalTime, 1.0/(time.time()-start)))
